/*
 * Copyright 2024, cafeina <cafeina@world>
 * All rights reserved. Distributed under the terms of the MIT license.
 */
#ifndef __mConstant_h_
#define __mConstant_h_

#define ERASER_FROM_DOOM        'prx8'
#define APPLY_EVERYTHING        'ere9'
#define QUIT_TEAM_MONITOR       'Dsab'

#define M_AUTHMTHD_SYSTEM       'aths'
#define M_AUTHMTHD_APPACC       'atha'
#define M_AUTHOPTS_THRSHD       'atht'
#define M_AUTHOPTS_ERRSNZ       'athz'

#define M_APPUSER_MODIFY        'usmd'
#define M_APPUSER_REMOVE        'usrm'
#define M_APPUSER_SEL           'usel'
#define M_APPUSER_IVK           'uivk'
#define TEXTADDUSER             'pro2'
#define TEXTADDPASS             'pro3'
#define USERLIST                'pro4'
#define CHANGE_LOGIN            'asd4'

#define M_BGCOLOR_CHANGED_R     'prx4'
#define M_BGCOLOR_CHANGED_G     'prx5'
#define M_BGCOLOR_CHANGED_B     'prx6'
#define M_BGCOLOR_DEFAULTS      'erea'
#define M_BGMODE_NONE           'rad0'
#define M_BGMODE_FOLDER         'rad1'
#define M_BGMODE_LIST           'rad2'
#define M_BGMODE_SINGLE         'rad3'
#define M_BGIMG_SINGLEPATH      'proa'
#define M_BGIMG_FOLDERPATH      'pro5'
#define M_BGIMG_LISTPATH        'prob'
#define M_BGIMG_BROWSE_SINGLE   'asd2'
#define M_BGIMG_BROWSE_FOLDER   'asd3'
#define M_BGIMG_BROWSE_LIST     'asd5'
#define M_BGIMG_SNOOZE_CHANGED  'snzs'
#define M_BGIMG_ADJUST_KEEP     'fcnt'
#define M_BGIMG_ADJUST_X        'fitx'
#define M_BGIMG_ADJUST_Y        'fity'
#define M_BGIMG_ADJUST_X_Y      'fixy'
#define M_BGIMG_ADJUST_STRETCH  'fitt'
#define STATIC_CHANGED          'a335'
#define FOLDER_CHANGED          'a333'
#define LIST_CHANGED            'a334'
#define TEXT_IMAGEPATH          'pro5'
#define BUTTON_DEFAULTPATH      'pro9'
#define DEFAULT_COLORS          'erea'
#define FOLDER_SELECTED         'asds'

#define BUTTON_CLICKED          'pr21'
#define BOOL_CLOCK              'ere5'
#define CLOCKPLACEMENT_X        'ere6'
#define CLOCKPLACEMENT_Y        'ere7'
#define DEFAULT_CLOCK_COLORS    'eraa'
#define DEFAULT_PLACE           'asda'
#define SIZE_SLIDER_CHANGED     'asdy'
#define M_BGCOLOR_CHANGED       'prxx'
#define M_CLOCK_COLORCHANGED    'erex'

#define LANG_0                  'aaaa'
#define LANG_1                  'aann'
#define LANG_2                  'aan1'
#define LANG_3                  'aan2'
#define LANG_4                  'aan3'
#define LANG_5                  'aan4'
#define LANG_6                  'aan5'

#define CHECK_BUTTONS           'ada2'
#define CHECK_USERBUTTON        'wada'
#define M_ITEM_SELECTED         'isel'

#define M_BOOL_SESSION          'cbsb'
#define M_BOOL_INFO             'cbsi'
#define M_BOOL_KILLER           'cbks'
#define M_BOOL_EVTLOG           'cbel'
#define M_BOOL_PWDLESS          'cbpw'
#define M_EVTLOG_CLEAR          'lgcl'

#define M_UTIL_ADDBOOT          'boot'
#define M_UTIL_REMBOOT          'woot'

#endif /* __mConstant_h_ */
